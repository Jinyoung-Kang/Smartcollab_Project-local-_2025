const Dashboard = ({ user }) => {
                ...
                Realtime.connect(
                    team.id, {
                        ...
                        onEditorChange: (change) => {
                            if (change.editor !== user.username) {
                                setEditingFile(prev => (prev && prev.id === change.fileId) ? { ...prev, content: change.content } : prev);
                            }
                        },
                    },
                );
    ...
    const handleSendMessage = (message) => { if (viewContext.type === 'team') { Realtime.sendMessage(selectedTeamContext.id, message); } };
    ...
    const handleSummarize = () => { if (!editingFile) return; setAiModalTitle("요약 결과"); setAiResultContent(""); setIsAiModalOpen(true); API.summarizeFile(editingFile.id).then(result => setAiResultContent(result)).catch(err => setAiResultContent("오류 발생: " + err.message)); };
