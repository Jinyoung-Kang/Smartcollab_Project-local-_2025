...
const EditorView = ({ file, onContentChange, onClose, onSave, onSummarize, onTranslate }) => {
    return (
        <div className="bg-white rounded-lg shadow h-full flex flex-col">
            <div className="p-4 border-b flex justify-between items-center">
                <h3 className="font-bold text-lg">{file.name}</h3>
                <div className="flex items-center space-x-2">
                    <button onClick={onSummarize} className="px-3 py-2 bg-green-500 text-white text-xs rounded-lg hover:bg-green-600">
                        요약하기
                    </button>
                    <button onClick={onTranslate} className="px-3 py-2 bg-purple-500 text-white text-xs rounded-lg hover:bg-purple-600">
                        영어로 번역
                    </button>
            ...
            <textarea
                ...
                className="w-full h-full p-4 resize-none border-none focus:ring-0 font-mono"
                placeholder="파일 내용..."
            />
        </div>
    );
...
const CollaborationPanel = ({ user, teamContext, permissions, onInviteClick, onRemoveMember, onEditPermissions, onDelegateLeadership, onDeleteOrLeaveTeam, onClearChatHistory, onReloadMembers, chatMessages, onSendMessage, onFileUploadSuccess }) => {
    const [activeTab, setActiveTab] = React.useState('members');

    const commonTabStyle = "px-4 py-2 text-sm font-medium";
        ...
        <aside className="col-span-3 bg-white rounded-lg shadow flex flex-col">
            <div className="border-b">
                <nav className="-mb-px flex space-x-4 px-4">
                    ...
                    <button onClick={() => setActiveTab('chat')} className={`${commonTabStyle} ${activeTab === 'chat' ? activeTabStyle : inactiveTabStyle}`}>
                        채팅
                    </button>
                </nav>
            </div>
    ...
    <aside className="col-span-3 bg-white p-4 rounded-lg shadow">
        ...
        <div>
            ...
            <ul className="space-y-3 text-sm">
                <li>
                    <div>
                        <p className="text-gray-800">`요구사항 명세서.docx`를 수정했습니다.</p>
                        <p className="text-xs text-gray-400">15분 전</p>
                    </div>
                </li>
                <li>
                    <div>
                        <p className="text-gray-800">`졸업작품 팀`에 'teammate1'님을 초대했습니다.</p>
